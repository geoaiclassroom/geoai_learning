...
class ResNet18UNetPP(nn.Module):
    def __init__(self, freeze_backbone=False, in_channels=2, num_classes=3):
        ...
        # Blocs de convolution imbriqués
    ...
    def forward(self, x):
        ...
        # Connexions de saut denses UNet++
...
def multiclass_focal_loss(
    ...
    if targets.numel() == 0:
        # pas de pixels valides dans le batch
    ...
    alpha_t = alpha_t.sum(dim=-1)  # car seulement un élément non-zéro par ligne
...
def multiclass_dice_loss(
    ...
    # Convertir les cibles → one-hot
    valid_mask = targets != ignore_index

    if valid_mask.sum() == 0:
        return logits.sum() * 0.0

    # Temporairement définir les pixels ignorés à 0 pour que one_hot fonctionne
...
def update_confusion_matrix(cm, y_true, y_pred, ignore_index):
    ...
    if y_true.numel() == 0:
        return cm, torch.zeros((num_classes, num_classes))  # rien à mettre à jour
...
def run_training(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        # suivre le meilleur modèle par iou global pondéré
        va_iou_pos = va_iou[0]
        va_iou_count = 1
        for va in va_iou[1:]:
            va_iou_pos += 2.0 * va
