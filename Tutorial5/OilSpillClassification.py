...
class OilSpillDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        img = img[[1, 0], :, :] # le jeu de données de nappes de pétrole est vv+vh tandis que satlas pré-entraîné attend vh+vv
        ...
        #         count=2,
        #         dtype="float32",
        #         transform=Affine.identity(),  # optionnel mais explicite
        #         crs=None  # explicitement pas de CRS
        # ) as dst:
        #     dst.write(img)
...
class ResNet18Classifier(nn.Module):
    def __init__(self, freeze_backbone=False, in_channels=2, num_classes=3):
        ...
        # Tailles de canaux de ResNet-18
        # conv1: 64 ch
        # layer1: 64 ch (1/4)
        # layer2: 128 ch (1/8)
        # layer3: 256 ch (1/16)
        ...
        # Après conv, la carte de caractéristiques est [B,c4,H/32,W/32] → avg-pool vers [B, c4]
        # Puis linéaire vers des logits scalaires
        # Suivant l'implémentation originale : https://docs.pytorch.org/vision/stable/_modules/torchvision/models/resnet.html#resnet18
...
# Ceci est basé sur le modèle fondamental SATLAS spécifiquement pré-entraîné sur des images S1
# Référence à https://github.com/allenai/satlaspretrain_models#sentinel-1-pretrained-models
class SatlasClassifier(nn.Module):
    def __init__(self, freeze_backbone=True, in_channels=2, num_classes=3):
        ...
        # Tailles de caractéristiques Satlas :
        # feats = self.backbone(x) -> [x1,x2,x3,x4] avec
        # x1: [B,128,,H/4,W/4], x2: [B,256,H/8,W/8], x3: [B,512,H/16,W/16], x4: [B,1024,H/32,W/32] (H/32)
        c4 = 1024

        # Après conv, la carte de caractéristiques est [B,1024,H/32,W/32] → pool vers [B, 1024]
        # Puis une couche résiduelle et ensuite linéaire vers des logits scalaires
    ...
    def forward(self, x):
        feats = self.backbone(x)
        feats = feats[-1]  # carte de caractéristiques la plus profonde
...
def save_checkpoint(path, model, optimizer, scheduler, epoch, best_val):
    ckpt = {
        "epoch": epoch,  # prochaine époque à exécuter (convention courante)
        "best_val": best_val,
...
# Concevons maintenant nos fonctions de "perte".
...
def print_trainable(model):
    # Juste pour que nous puissions être sûrs que les bons paramètres sont gelés
...
def update_confusion_matrix(cm, y_true, y_pred):
    ...
    # Déplacer vers le CPU car cm est en cpu
...
# Gérons tout le processus d'entraînement
def run_training(
        ckpt_dir: str,
        run_dir: str,
        model: nn.Module,
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        # suivre le meilleur modèle par F1 global pondéré
        va_f1_pos = va_f1[0]
        va_f1_count = 1
        for va in va_f1[1:]:
            va_f1_pos += 2.0 * va
...
def predict_probs_cam(model, device, loader, out_mask_dir, model_type):
    ...
    if model_type == "satlas":
        cam_extractor = SmoothGradCAMppSatlas(model, target_layer=model.backbone.backbone.backbone.features[-3], std=0.01) # x3 (avant-dernier en profondeur)
    else:
        cam_extractor = SmoothGradCAMpp(model, target_layer=model.backbone[6], std=0.01) # layer3 (avant-dernier en profondeur)
    ...
    for imgs, class_ids, imgs_path in loader:
        ...
        class_ids = class_ids.to(device)
        logits = model(imgs)
        class_idxs = logits.argmax(dim=1).detach().cpu().tolist()  # forme [B]
        cams_all = cam_extractor(class_idx=class_idxs, scores=logits)
        # Couche cible unique -> cams est une liste de longueur 1
        cams_all = cams_all[0]  # forme : [B, H/16, W/16]
        for b in range(B):
            ...
            cmap = get_cmap("jet")
            # Redimensionner le masque et appliquer la carte de couleurs
            ...
            if imgs_path[b].endswith('.tif'):
                ...
                with rasterio.open(os.path.join(out_mask_dir, 'cam_' + name + '.tif'), "w", **profile_out) as dst:
                    # rasterio attend [C,H,W]
                    dst.write(cams.transpose(2, 0, 1))
            else:
                img = Image.fromarray(cams)  # mode déduit comme RGB
...
# Ajouter une fonction pour effectuer une inférence simple à partir d'un modèle entraîné
