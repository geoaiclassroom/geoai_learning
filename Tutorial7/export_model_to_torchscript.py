"""
Exporter un modèle SegmentationUnetPlus entraîné vers TorchScript.

Ce script gère à la fois le développement local (module geo_deep_learning local)
et les scénarios de packages installés. Il charge un checkpoint et le convertit en
TorchScript pour le déploiement.

Le modèle exporté inclut une normalisation intégrée, donc votre bibliothèque d'inférence
n'a qu'à passer des tenseurs d'images brutes et obtiendra directement les prédictions.
