...
class CSVDataset(NonGeoDataset):
    ...
    * csv files are named after the data split e.g. 'trn.csv', 'val.csv', 'tst.csv'
    ...
    def __init__(
        ...
        convert_255_to_1_mask: bool = False
    ) -> None:
        """
        ...
        self.convert_255_to_1_mask = convert_255_to_1_mask
    ...
    def _load_mask(self, index: int):
        ...
        if self.single_label:
            with rio.open(mask_path) as mask:
                lbl = mask.read(1)
                if self.convert_255_to_1_mask:
                    lbl = np.where(lbl == 255, 1, lbl) # 255 to 1 (so we have 0 and 1)
                lbl = torch.from_numpy(lbl).long()
            return lbl, mask_name
        else:
            with rio.open(mask_path) as mask:
